...
if modulo != 0:
	...
	# to get to an even number of remaining threads to distribute 
	if modulo % 2 != 0:
		all_section_number_threads[0] += 1 
		modulo -= 1
	
	left_index = 0
	right_index = len(all_section_number_threads) - 1

	# determine if even or odd number of sections 

	# even number of sections 
	if (len(all_section_number_threads) % 2 == 0):
		# determine true halfway point 
		...
		center = len(all_section_number_threads) // 2 
		while modulo > 0: 
			all_section_number_threads[left_index] += 1
			all_section_number_threads[right_index] += 1
			left_index += 1
...
for section_length in all_section_number_threads:
	...
	for group in grouped: 
		final_sections[current_section_number] += group[0] + " " + str(len(list(group[1]))) + " __ "  
